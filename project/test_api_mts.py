...
@allure.description("Тест проверяет, что запрос к несуществующему товару корректно возвращает статус 404.")
...
@allure.description("Запрос корзины возвращает актуальный список товаров и общую стоимость.")
def test_get_cart():
    client = ApiClient()
    assertions = ApiAssertions()
...
@allure.description("Тест проверяет наличие обязательных HTTP-заголовков и их корректность.")
