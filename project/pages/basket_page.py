...
class BasketPage(BasePage):
    ...
    MODAL_LIMIT_TEXT = (By.XPATH, "//div[contains(@class, 'dialog-modal__confirm-text') and contains(text(), 'юридическое лицо')]")
    ...
    def wait_for_basket_to_load(self):
        ...
        try:
            self.wait.until(
                EC.presence_of_element_located((By.CSS_SELECTOR, ".basket-item, .basket-empty, .empty-basket"))
            )
        except:
            pass
        return self
    ...
    def set_quantity_and_trigger_limit(self, quantity):
        ...
        self.driver.execute_script("arguments[0].select();", qty_input)  # Выделяет весь текст
