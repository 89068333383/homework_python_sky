...
class SearchResultsPage(BasePage):
    ...
    MODAL_TITLE = (By.CSS_SELECTOR, "div.mtsds-modal-page__header p.mtsds-modal-page__header-title")
    MODAL_TITLE_ALT = (By.CSS_SELECTOR, ".modal-header, .mtsds-modal-page__header")
    BASKET_BUTTON = (By.CSS_SELECTOR, "a[href='/personal/basket'] .mtsds-button__text-container")
    BASKET_BUTTON_ALT = (By.XPATH, "//a[contains(@href, '/personal/basket')]//*[contains(text(), 'В корзину')]")
    ...
    def wait_for_products(self):
        ...
        try:
            self.short_wait.until(EC.presence_of_all_elements_located(self.PRODUCT_CARDS))
        except:
            self.short_wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, ".product-item, .product, [data-testid='product-card']")))
        return self
    ...
    def find_product_by_keyword(self, keyword):
        ...
        if not product_links:
            product_links = self.driver.find_elements(By.CSS_SELECTOR, "a[href*='/product/'], a[href*='/catalog/']")
        ...
        cards = self.driver.find_elements(By.CSS_SELECTOR, ".product-card, .product-item, [data-testid='product-card']")
        for card in cards:
            card_text = card.text
    ...
    def get_buy_button_in_card(self, product_link):
        ...
        try:
            card_container = product_link.find_element(By.XPATH, "./ancestor::div[contains(@class, 'product-card')]")
        except:
            card_container = product_link.find_element(By.XPATH, "./ancestor::div[contains(@class, 'product')]")
        
        # Ищем кнопку "Купить"
        buy_buttons = card_container.find_elements(By.CSS_SELECTOR, "button.mtsds-button, button[data-testid='buy-button']")
        
        for button in buy_buttons:
            try:
                btn_text = button.find_element(By.CSS_SELECTOR, ".mtsds-button__text-container, .button__text, span").text
    ...
    def add_product_to_cart(self, product_link):
        ...
        modal_locators = [
            ...
            (By.XPATH, "//*[contains(text(), 'Товар в корзине') or contains(text(), 'добавлен в корзину')]")
        ]
        ...
        if not modal_found:
            ...
            try:
                cart_counter = WebDriverWait(self.driver, 3).until(
                    EC.presence_of_element_located((By.CSS_SELECTOR, ".cart-counter, .basket-counter, [data-testid='cart-count']"))
                )
